...
The residents of the home have the thermostat set to hold at 73 degrees Fahrenheit.
The residents leave for vacation, but forget to turn the thermostat off. After 24 hours
of home inactivity, the system sends an SMS to the resident to let them know that
the thermostat has been set while nobody has been home for 24 hours. The resident
replies to the SMS which then turns the thermostat off.
...
it is escalated to the security company for them judge the problem and possible
...
% TODO :: put in a flow chart of the escallation system that can be used here.
...
minimal training on how to arm and disarm the system. Only parents, or the
owner of the house, need to be fully trained on the entire system. The system
should be intuitive and seemless in day to day life. Input and output devices
should be simple to use. 

% More about why the system needs to be intuitive
...
and data. The controller should have a scheduler that will prioritize requests
from sensors based on criticality. Event driven sensors need to have
confirmation from the base controller before dismissing the event. The
controller and server should be able to communicate quickly in order to respond
to events in real time. Data between components should be minimized to speed up
response time.

\section{Supportability}
Several levels of support are required. The system should be easily supported
remotely for most minor issues. For major issues, parts should be sent back and
replaced. Other issues should be handled by the discretion of the security
company.

\section{Implementation}
Services should be offered by the security company or a contractor to install
this system professionally for best results. An option should also be available
for self-installation at the home owner's discretion. A list of sensors
supported by the system needs to be available and these sensors should be
...
The system should be relatively secure. It needs to be able to
withstand most attempts to tamper with it by either alerting the users when
sensors are missing or malfunctioning or by having enclosures and packaging
capable of withstanding attack. Wireless information transmitted between nodes
should be encrypted to ensure that traffic sniffing cannot be used against the
users.

Data security also needs to be accounted for. Any and all data stored off site
...
accessible by a select group of users.

%\section{Interface}

\section{Quality}
% Downtime is completely arbitrary.
The system must be reliable and be able to still operate even with missing
sensors. Faults should be trapped and the user alerted of these occurances.
...
should be able to start reporting their status as soon as they can connect to
the main controller. Connections between base controller and sensors should be
...
\section{Packaging}
Packaging requirements varies for each component. Each sensor should
...
battery level. As mentioned previously, sensors located outdoor need to be
