...
The class diagram for the first controller, the SensorController, and the
sensors are shown in Figure~\ref{fig:sensor_controller_class_diagram}. This
diagram also describes the EventType enumeration which is omitted from the
other diagrams for brevity The SensorController aggregates a list of sensors,
which is fetched from the EventManager. This controller can explictely poll
sensors, handle the sensor input data, and check the sensor statuses. It sends 
events to the other controllers through the event manager after deciding how an
...
\restylefloat{figure}
\begin{figure}[p]
    \caption{SensorController and Sensor Class Diagram}
